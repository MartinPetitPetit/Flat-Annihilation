...
void UIManager::renderHUD(const Player* localPlayer, const std::vector<Unit*>& selectedUnits)
{
    ...
    else if (selectedBuilding != nullptr && selectedBuilding->getType() == BuildingType::TownCenter)
    {
        // Barre de capacités du Town Center.
        drawHUDRect(buildRect, { 10, 30, 10, 180 }, borderGreen);

        // Bouton produire collecteur.
        SDL_Rect collectorBtn = { buildRect.x + 8, buildRect.y + 10, 110, 50 };
        drawHUDRect(collectorBtn, { 20, 60, 20, 180 }, borderGreen);
        drawHUDText("Collector", collectorBtn.x + 6, collectorBtn.y + 8,  { 100, 255, 100, 255 });
        drawHUDText("w20 f40",   collectorBtn.x + 8, collectorBtn.y + 28, { 180, 140, 80,  255 });
    }
    else
    {
